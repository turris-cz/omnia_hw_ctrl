...
enum i2c_commands {
    ...
    CMD_PERIPH_CONTROL         = 0x0F, /* only for Omnia board version 32 and higher ! */
    CMD_GET_PERIPH_RESET_STATUS         = 0x10 /* only for Omnia board version 32 and higher ! */
...
=== CMD_PERIPH_CONTROL
* Reset of different peripheral devies on the board
* Write only
* Bit overview:
...
* Example of a setting of the peripheral control byte (release reset of PCIE slot 0):
** "i2cset 1 0x2A 0x0F 0x0800 w"
*** 1 -> i2cbus number
*** 0x2A -> I2C address of the slave
*** 0x0F -> "address of the register" = command
*** 0x0800 -> do light reset – the lower byte is a mask (set particular bit to "1" to use a value defined in the higher byte)
*** w -> word data type
