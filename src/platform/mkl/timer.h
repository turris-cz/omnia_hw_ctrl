...
{
	switch (nr) {
	case 0: sys_clk_config(TPM0_Slot, on); break;
	case 1: sys_clk_config(TPM1_Slot, on); break;
	case 2: sys_clk_config(TPM2_Slot, on); break;
	default: unreachable();
	}
}
					   ...
					   uint32_t freq, uint8_t irq_prio)
{
	...
	sys_enable_irq_with_prio(tpm_irqn(nr), irq_prio);
}
